...
In this section I compute the optimal Taylor rule coefficients numerically for a simulation of both the rational expectations and learning versions of the model.\footnote{To concentrate on the intuition, in this section I implement the learning algorithm such that only the constant is learned. The general formulation has qualitatively similar features but is more impacted by small-sample concerns prevalent in simulations.} Table \ref{calibration2} reproduces the calibration of Table \ref{calibration1}, highlighting in blue differences between parameter values. I make the following changes. For this section, I shut off the monetary policy parameters $\lambda_i$, $\lambda_x$ and $\psi_x$ in order to focus on the role of inflation in the central bank's problem and thus on the optimal choice of inflation aggressiveness, $\psi_{\pi}$. As usual, this choice is motivated by the ease of interpretation. 
