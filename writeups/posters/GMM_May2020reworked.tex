\documentclass[22pt, a1paper, landscape]{tikzposter}
\usepackage[utf8]{inputenc}

\title{\LARGE{\bf{Monetary Policy \& Anchored Expectations - An Endogenous Gain Learning Model}}}
\author{\LARGE{Laura G\'ati}}
\date{\today}
\institute{\Large Boston College}
...
\item Here: private sector does not know (\ref{obs_RE}) $\rightarrow$ instead postulate
\begin{equation}
y_t = \bar{y} + g s_t \label{explicitPLM}
\end{equation}

\item General case: estimate $(\bar{y}, g)$ using (\ref{state}) \& observed states
\item Special case: private sector estimates only the long-run mean of inflation:

\begin{equation}
\hat{\E}_t \pi_{t+1} =\textcolor{brownreddark}{\bar{\pi}_{t-1}} + g_1h_1s_t  \label{PLM_fcst_general}
\end{equation}
\
\end{itemize}
...
Private sector updates estimate of mean inflation using recursive least squares
\begin{equation}
...
\section{Intertemporal tradeoffs under anchoring expectations}

\begin{result} \hspace{0.5cm} Target criterion under anchoring
\begin{align*}
...
\begin{result} \hspace{0.5cm} For any adaptive learning scheme, the discretion and commitment solutions of the Ramsey problem coincide.  The solution qualitatively resembles discretion and is thus not subject to the time inconsistency problem.
\end{result}

\section{Quantitative implications for central banking}
