...
\title{Materials 29 - Making an expectations equation residual}
\author{Laura G\'ati} 
\date{\today}
\maketitle
...
CUSUM-equations:
\begin{align}
\omega_t & = \omega_{t-1} + \kappa k_{t-1}^{-1}(f f' -\omega_{t-1}) \\
\theta_t & = \theta_{t-1} + \kappa k_{t-1}^{-1} (f'\omega_t^{-1}f - \theta_{t-1}) \\
k_t & = \mathbb{I}_{\theta_t < \tilde{\theta}}(k_{t-1}+1) +(1-\mathbb{I})\bar{g}^{-1}
\end{align}

Instead, I would like to use the smooth anchoring equation because it only adds one extra equation and because the target criterion is formulated in terms of it. But for that I need a functional form that works, i.e. has at least three desirable features and is robust to parameter values and input sequences. Below is one that does an ok job but is very sensitive to parameter values and input sequences:
\begin{equation}
k_t^{-1} = \rho k_{t-1}^{-1} + \gamma fe_{t|t-1}^2
\end{equation}
with $\rho=0.9, \gamma=0.001$ 
Necessary features:
\begin{enumerate}
\item generally decreasing
\item can jump up from time to time when forecast errors are large
\item does not decrease below zero
\end{enumerate}

Report success! For this smooth gain function, I've been able to do 2 things:
\begin{itemize}
\item Input exogenous gain sequence ($\{i_t, k_t\}$) and treat TR  (\ref{TR}) \& LOM gain (\ref{A6}) as residual: \\
$\rightarrow$ this does solve even if agents don't know the Taylor rule, but only from a small neighborhood of the solution.
\item Input exogenous forecast error sequence ($\{i_t, fe_{t|t-1}\}$) and treat TR  (\ref{TR}) \& LOM FE (\ref{A7}) as residual: $\rightarrow$ this solves even if agents don't know the Taylor rule, and from a large neighborhood. The only back-draw is that it takes a while (128 iterations, almost 4 minutes). 
\end{itemize}
