...
\title{Materials 31 - Parameterized expectations against value iteration}
\author{Laura G\'ati} 
\date{\today}
\maketitle
...
\item Since equations (\ref{A5})-(\ref{A7}) involve past shocks, I need to take expectations regarding what node of the state-space we're coming from (``backward expectations'').
\item How to compare the results? For value iteration, I get decision rules for $i, \bar{\pi}$ and $k^{-1}$ given grid values, while for projection I get a sequence for these given a simulated time series of shocks. I'm trying to pass the same time series of states from the projection result and see if I get the same interest rate sequence.
\end{itemize}


\begin{figure}[h!]
\subfigure[Observables, Taylor rule]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath implement_anchTC_obs_TR}}
\hfill % this is great to intro dpace between subfigures
\subfigure[Inverse gain, Taylor rule]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath implement_anchTC_invgain_TR}}
\subfigure[Observables, target criterion]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath implement_anchTC_obs}}
\hfill % this is great to intro dpace between subfigures
\subfigure[Inverse gain, target criterion]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath implement_anchTC_invgain}}
\caption{Parameterized expectations solution}
\end{figure}
