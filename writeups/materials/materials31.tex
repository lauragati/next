...
\item How to compare the results? For value iteration, I get decision rules for $i, \bar{\pi}$ and $k^{-1}$ given grid values, while for projection I get a sequence for these given a simulated time series of shocks. I'm trying to pass the same time series of states from the projection result and see if I get the same interest rate sequence. In math:
\begin{align}
V(X^{pe}) & = L(i) + \beta V(X^{pe}) \\
i^*: \quad L(i) & = (1-\beta)V(X^{pe})
\end{align}
...
\caption{Parameterized expectations against value iteration solution}
\end{figure}
