...
\title{Materials 31 - Parameterized expectations (PE) against value iteration (VFI)}
\author{Laura G\'ati} 
\date{\today}
\maketitle
...
The value function problem: the planner chooses $\{i_t\}_{t=t_0}^{\infty}$ to minimize
\begin{align}
V(\mathbf{x}_t,t)& = \min \bigg\{ (\pi_t^2 + \lambda_x x_t^2) + \beta \E_t V(\mathbf{x}_{t+1},t+1) \bigg\} \\
& \text{s.t. to model equations, (\ref{A1})-(\ref{A8})}
\end{align}
where $\mathbf{x}_t \equiv (k_{t-1}^{-1}, \bar{\pi}_{t-1}, r_t^n, u_t, r^n_{t-1},u_{t-1})$. $V$ is thus $\mathcal{R}^6 \rightarrow \mathcal{R}$.

Questions:
\begin{itemize}
\item How to compare the results? For VFI, the policy function is \texttt{size(grid)} while for PE, it is $1\times T$, where $T$ is the length of a given simulated time series of shocks. I'm trying to pass the same time series of states from the PE result  ($X^{pe}$), obtain the value function, and try to back out the associated $1\times T$ policy from there. That is
\begin{align*}
V(X_t^{pe}) & = L(i) + \beta \E_tV(X_{t+1}^{pe}) \numberthis \\
& \text{Since I had a hard time taking the $\E$, I cheated by computing} \\
L(i) & = (1-\beta)V(X_t^{pe}) \numberthis \\
& \text{Given}\quad  L(i), \quad i_t = L^{-1}(i)
\end{align*}
\item If I have $L(i)$, I need to solve $L(i_t)  = \pi_t(i_t)^2 + \lambda_x x_t(i_t)^2$ at each $t$, which is a quadratic in $i_t$. Difficult to discriminate between the roots, which sometimes, for some $t$, are complex.
\end{itemize}
...
Notes for me:
\begin{itemize}
\item Since equations (\ref{A5})-(\ref{A7}) involve past shocks, I actually have 2 additional states. If I instead take expectations regarding what node of the state-space we're coming from (``backward expectations''), then:
\item I'm using a $4\times4\times2\times2$ grid, and cubic spline interpolation for the value function. This takes 260 seconds. I'm sure I could speed things up by doing policy iteration instead. 
\end{itemize}
\begin{figure}[h!]
\subfigure[]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath pe_vfi_initPE}}
\hfill % this is great to intro dpace between subfigures
\subfigure[]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath pe_vfi_initrand}}
\caption{Parameterized expectations against value iteration solution, 4 states (ignore past states)}
\end{figure}
