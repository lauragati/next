...
Then I estimate $\beta$ using GMM, targeting the autocovariance structure of inflation, the output gap and the nominal interest rate (federal funds rate) in the data. 
...
\item I try to set the loss very large if the gain explodes or turns negative, but I worry that it still does so off the grid, especially as the PEA's fsolve cannot solve now. 
\end{enumerate}
...
\caption{$k^{-1}_t$ as a function of $k^{-1}_{t-1}$ and $fe_{t|t-1}$ given $\hat{\beta}^{GMM} > 0$}
...
\caption{$k^{-1}_t$ as a function of $k^{-1}_{t-1}$ and $fe_{t|t-1}$ given $\hat{\beta}^{GMM}$ (not restricted $>0$)}
\label{fig_estmtd_anchor_fct}
\end{figure}

\clearpage
\section{Is the estimated LOM gain in the nonpermissible range for PEA?}
No, it doesn't look like it. If I fit an AR(1) (Equation \ref{A6}) to the estimated gain evolution, I obtain $(\rho_k, \gamma_k)=(0.6151, 0.0011)$ which results in the following relationship:
\begin{figure}[h!]
\includegraphics[scale=0.18]{\myFigPath analyze_estim_LOMgain_fitAR_14_Jun_2020}
\caption{AR(1) approximation to the estimated functional form of the gain}
\label{fitAR1}
\end{figure}

In particular, when doing PEA with the AR(1) specification for the gain, fsolve solved for my values of (0.5, 0.001). Rerunning PEA with the AR(1) specification and the values (0.6151, 0.0011) also works. So what's the problem with the PEA given approximated anchoring function?
