...
\begin{itemize}
\item Use value function iteration policy values to approximate the policy function function using a cubic spline: obtain the coefficients $\alpha$ in $\hat{i}(\alpha,X^{grid})$.
\item Plug in the history of states from PE: $\hat{i}(\alpha, X^{pe})$.
\end{itemize}

\begin{figure}[h!]
\includegraphics[scale=\myAdjustableFigScale]{\myFigPath materials32_policy}
\caption{Policy function for a particular history of states, $X^{pe}$}
\end{figure}
