...
\title{Materials 32 - Comparing results: parameterized expectations (PE) against value iteration (VFI)}
\author{Laura G\'ati} 
\date{\today}
\maketitle
...
Approach to map value function iteration solution to the solution of parametric expectations:
\begin{itemize}
\item Use value function iteration policy values to approximate the policy function using a cubic spline: obtain the coefficients $\alpha$ in $\hat{i}(\alpha,X^{grid})$.
\item Plug in the history of states from PE: $\hat{i}(\alpha, X^{pe})$.
\end{itemize}

\begin{figure}[h!]
\subfigure[$X_1^{pe}$]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath materials32_policy}}
\subfigure[$X_2^{pe}$]{\includegraphics[scale=\myAdjustableFigScale]{\myFigPath compare_value_pea_results_value_outputs_server32_accelerated_pea_outputs_30_May_2020_10_18_28}}
\caption{Policy function for two particular histories of states, $X^{pe}$}
\end{figure}

Things I haven't done yet (in priority order)
\begin{enumerate}
\item estimation of the functional form of the gain
\item relating the policy function to the Taylor rule more
\item optimal Taylor-rule coefficients using the smooth criterion
\item optimal time-varying Taylor-rule coefficients
\item Susanto's comment that the TIPS-based inflation expectation series needs to be corrected for liquidity premia
\end{enumerate}
