...
% di/dpibar
\def\fignameDiDpibar{analyze_opt_policy_ip04_Sep_2020}
...
\def\fignameCBlossnilnil{plot_sim_loss_approx_pretty_losses_again_critsmooth_constant_only_pi_only_lamx0_lami0_2020_09_03}
\def\fignameCBlossonenil{plot_sim_loss_approx_pretty_losses_again_critsmooth_constant_only_pi_only_lamx1_lami0_2020_09_03}
\def\fignameCBlossnilone{plot_sim_loss_approx_pretty_losses_again_critsmooth_constant_only_pi_only_lamx0_lami1_2020_09_03}
\def\fignameCBlossoneone{plot_sim_loss_approx_pretty_losses_again_critsmooth_constant_only_pi_only_lamx1_lami1_2020_09_03}
\def\fignameCBlossbaseline{plot_sim_loss_approx_pretty_losses_again_critsmooth_constant_only_pi_only_lamx0_05_lami0_2020_09_03}
...
\def\fignameIRFanchored{RIR_anch_psi_pi1_5_command_IRFs_approx_pretty_2020_09_07}
\def\fignameIRFunanchored{RIR_unanch_psi_pi1_5_command_IRFs_approx_pretty_2020_09_07}

% Cross-sectional average gains for various psi_pi
\def\fignameGainPsiSmall{gain_sim_psi_pi_1_01_command_IRFs_approx_pretty_2020_09_07}
\def\fignameGainPsiMedium{gain_sim_psi_pi_1_5_command_IRFs_approx_pretty_2020_09_07}
\def\fignameGainPsiBig{gain_sim_psi_pi_2_command_IRFs_approx_pretty_2020_09_07}

% IRFs unanchored for various psi_pi
\def\fignameIRFpsipiSmall{RIR_unanch_psi_pi1_01_command_IRFs_approx_pretty_2020_09_07}
\def\fignameIRFpsipiMedium{RIR_unanch_psi_pi1_5_command_IRFs_approx_pretty_2020_09_07}
\def\fignameIRFpsipiBig{RIR_unanch_psi_pi2_command_IRFs_approx_pretty_2020_09_07}
...
The scenario that the Fed seeks to avoid, then, is one of expectations unanchoring and resulting in a self-fulfilling downward spiral in inflation. Since the Fed sees ``[its] actions to achieve both sides of [its] dual mandate [as] most effective if longer-term expectations remain well anchored at 2 percent,''\footnote{Ibid.} it is explicitly looking for the appropriate change in the conduct of monetary policy to achieve its target of anchored expectations.

This paper provides the first formal theory of optimal monetary policy in an environment where expectations may become unanchored. In my behavioral model of expectation formation, the notion of anchoring is the stability of the private sector's expectations of mean inflation over the long run, exactly corresponding to Chairman Powell's description above. As in \cite{carvalho2019anchored}, whether expectations are anchored or not depends on the size of the learning gain, the weight that the public places on current forecast errors when updating its expectation of mean inflation. Since the size of the gain depends on forecast errors, the central bank's interest-rate setting interacts with expectation formation, leading to the anchoring or unanchoring of expectations.
...
where $b(\cdot)$ is a second order spline basis and $\alpha$ is a vector of approximating coefficients. I estimate $\alpha$ by simulated method of moments \`a la \cite{lee1991simulation}, \cite{duffie1990simulated} and \cite{smith1993SMM}. I target the autocovariance structure of CPI inflation from the Bureau of Labor Statistics (BEA), the output gap, the federal funds rate from the Board of Governors of the Federal Reserve System and 12-month-ahead inflation forecasts from the Survey of Professional Forecasters (SPF).\footnote{The output gap measure is constructed as the difference between real GDP from the Bureau of Economic Analysis (BEA) and the Congressional Budget Office's (CBO) estimate of real potential output.} The dataset is quarterly and ranges from 1981-Q3 to 2020-Q1. App. \ref{SMM} contains a detailed description of the estimation methodology.
...
But Panel (a) of Fig. \ref{fig_loss} has a more surprising implication too. As opposed to the RE model, in the anchoring model, the losses caused by tolerating too much inflation volatility cannot be eliminated by raising $\psi_{\pi}$ infinitely. In fact, the loss goes to infinity if $\psi_{\pi}$ is raised above 2. This means that beyond a threshold, raising $\psi_{\pi}$ actually increases inflation volatility, instead of decreasing it. How does this come about?

The mechanism is the following. As the IS-curve of Equation (\ref{NKIS}) makes explicit, the private sector relies on expectations of not just future inflation and output gaps, but also of interest rates when choosing its actions today. A high $\psi_{\pi}$, together with the assumption that the private sector knows the Taylor rule, means that if conditions today cause the private sector to expect high inflation in the future, the private sector will also expect high interest rates down the line.\footnote{The same thing happens when the private sector does not initially know the Taylor rule because its main features are learned very fast.} Thus, the entire term structure of expectations is shifted, amplifying the feedback between expectations and outcomes, and resulting in higher fluctuations in the current period.\footnote{The impulse responses in App. \ref{app_oscillations} illustrate the relationship between $\psi_{\pi}$ and volatility, confirming the present intuition.}
...
For the empirically relevant weights on the interest rate and the output gap, 0 and 0.05 respectively,\footnote{The value of 0.05 for $\lambda_x$ is the estimate of \cite{rotemberg1997optimization}. \cite{woodford2011interest} argues for $\lambda_i=0$ given that major central banks usually only have a mandate to stabilize inflation (and sometimes output), but not explicitly the interest rate.} the RE-optimal value, 2.2, is above the anchoring-optimal one of 1.1. (Second row of Table \ref{psi_pi_opt}.) This suggests that in practice, monetary policy specified as a Taylor rule should be \emph{less} aggressive on inflation than what would be optimal under rational expectations. 

This clearly goes counter to the conventional wisdom in the adaptive learning literature, as exemplified for example by \cite{orphanides2004imperfect}, that monetary policy should be more aggressive under learning than under RE to work against the positive feedback.
What explains the difference between the two predictions is the term structure of expectations. The majority of the adaptive literature specifies Euler-equation learning schemes, in which long-horizon expectations play no role. The IS- and Phillips relations in such a case are perfect analogues of the rational expectations equations, except that the expectations operator is different. By contrast, long-horizon learning as advocated by \cite{preston2005} and pursued here, implies that the entire term structure of expectations matters. 

It is indeed the fact that the entire term structure of expectations matters for outcomes today is what renders the loss function convex. In fact, the U-shaped loss function is the cause of the intertemporal volatility tradeoff already foreshadowed in Section \ref{implement}. The reason is that a U-shaped loss implies that the central bank can cause high volatility both if it is too aggressive or not sufficiently aggressive on inflation. It only wants to be as aggressive as necessary to keep expectations anchored, so that it does not need to intervene further and thereby induce fluctuations into beliefs and observables. This consideration corroborates the findings of Section \ref{implement} that the optimal interest rate sequence responds to the volatility tradeoff by making sure that expectations never become too unanchored. The central bank is indeed willing to pay with high short-run volatility to preserve a situation of anchored expectations and thus lower volatility in the long run.

But this implies that policy design via Taylor rule is so dovish because the central bank is unable to be aggressive on inflation in the situations when it wants to, and dovish otherwise. In other words, the time-invariant nature of the Taylor rule robs the central bank of the flexibility of being aggressive only when unanchoring expectations necessitate it. Instead, a constant inflation coefficient would imply being aggressive on inflation also when expectations are anchored, introducing fluctuations into the economy that might even unanchor expectations. 
