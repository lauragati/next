...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TO DO:
% 1. possibly redo all figures so they don't have captions embedded (might also be only for accepted papers, also, Nakata and Schmidt, AER, have them, so we're good.)
% 2. Check if there are ugly holes and fix them (App. PEA)
% 3. Search for undefined refs by "??"
%. 4. Data and Code Availability Policy (only if accepted)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       OWN PACKAGES       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{amsmath}
% \usepackage{amsthm} % this guy causes some problem with AEA style
\usepackage{amssymb}
\usepackage{bbm} % use mathbb 
\usepackage{thmtools,thm-restate} % looks like i dont need this
\usepackage{mathtools} % need for coloneqq
\usepackage{breqn} % automatic line breaking of displayed equations % clashes with something
% layout:
...
% \usepackage[capposition=top]{floatrow} % the floatfoot command needs this
% colors:
\usepackage{colortbl} % need this to define colors
% hyperlinks:
% \usepackage[]{hyperref} % need this for hyperlinks of cited papers
% \usepackage[font=footnotesize,labelfont=bf]{caption} % this makes the caption smaller and bold
% font:
...
\shortTitle{Monetary Policy \& Anchored Expectations}
\author{Laura G\'ati\thanks{%
Boston College, 140 Commonwealth Avenue, Chestnut Hill, MA 02467, gati@bc.edu. %Acknowledgements
}}
\date{\today}
\pubMonth{Month}
\pubYear{Year}
...
\begin{abstract}
    This paper analyzes monetary policy in a model that accounts for the unanchoring of inflation expectations. The extent of anchoring is determined by an adaptive learning mechanism in which the public's learning gain varies endogenously. Within an otherwise standard macro model with nominal rigidities, I find that optimal policy involves an aggressive interest rate response to expectations unanchoring. This stabilizes both inflation expectations and inflation. Furthermore, I estimate the model-implied relationship that determines the extent of unanchoring. The data suggest that the expectation process is nonlinear: expectations exhibit a higher sensitivity to large forecast errors than to smaller ones.
\end{abstract}
% cut down to be 99, ISNT THE SAME AS IN June DRAFT.
...
This paper studies the conduct of monetary policy in a New Keynesian model where the degree of expectations anchoring responds endogenously to economic conditions. The main contribution is providing a new theory of monetary policy in which expectations exhibit endogenously varying degrees of unanchoring. The key innovation in my model is the introduction of a continuously variable endogenous gain function, which allows my model to capture the time-varying sensitivity of long-run expectations to short-run conditions. I take the model to the data to quantify my novel anchoring mechanism, and use it to characterize optimal policy analytically and numerically.
    
Fig. \ref{rolling} provides some motivating evidence for continuously time-varying levels of unanchoring. It shows the regression coefficient of a rolling window regression of long-run inflation expectations on short-run forecast errors. Letting $\bar{\pi}^i_t$ denote the 10-year inflation expectation of forecaster $i$ from the Survey of Professional Forecasters (SPF), and $f^i_{t|t-1} \equiv \pi_t - \E^i_{t-1}\pi_t$ denote forecaster $i$'s one-year ahead forecast error, I run
    \begin{align}
...
where $w$ indexes windows of 20 quarters. Fig. \ref{rolling} plots the time series of the estimated coefficient $\hat{\beta}^w_1$, along with 90\% and 95\% confidence intervals.\footnote{Appendix \ref{app_rolling_pi} presents robustness checks. Appendix \ref{unanchoring_in_data} discusses alternative measures of long-run inflation expectations.}
    ...
    \begin{figure}
        \includegraphics[scale = \mySmallFigScale]{\myFigPath \fignameRolling}
...
My work is also related to the literature attempting to explain features of expectations data using departures from the full information rational expectations (FIRE) paradigm. This literature consists of two main lines of attack. The first is relaxing the assumption of full information. In this body of work, information is either not fully or not symmetrically available, or information acquisition is costly (\cite{mankiw2002sticky}, \cite{sims2003}, \cite{mackowiak2009optimal}, \cite{angeletos2009policy}). The second strand of this literature, to which this paper belongs, instead emphasizes that expectation formation departs from rational expectations. One example of this literature is \cite{sargent1999}, who explores the role of adaptive learning on the part of the Federal Reserve for the Great Inflation. A more recent example is \cite{bordalo2018diagnostic}. These authors complement the empirical results of \cite{coibion2015information} with new evidence to suggest that the pattern of over- and underreaction to news is consistent with diagnostic expectations instead of dispersed information. My model of anchored expectations is also consistent with these data facts. 
...
\begin{equation}
f_{a,t} \equiv \frac{1}{1-\alpha\beta}a_{t-1}  + b_{t-1}(I_3 - \alpha\beta h)^{-1}s_t \quad  \text{and} \quad  f_{b,t} \equiv \frac{1}{1-\beta}a_{t-1}  + b_{t-1}(I_3 - \beta h)^{-1}s_t  \label{fafb_anal}.
\end{equation}
...
\begin{table}
    \caption{Parameters calibrated from literature}
    ...
    \end{tabular}
    \begin{tablenotes}
        Parameters with an asterisk refer to sections of the paper where a Taylor rule is in effect.
    \end{tablenotes}
    \label{calibration}
\end{table}

\begin{table}
    \caption{Parameters set to match data moments}
...
\begin{figure}
    \subfigure[]{\includegraphics[scale=\myTinyFigScale]{\myFigPath \fignameFeSPF}}
    \subfigure[]{\includegraphics[scale=\myTinyFigScale]{\myFigPath \fignameGainSPF}}
    
    \caption{Forecast errors and implied gain in the SPF data}
    \begin{figurenotes}
        Forecast errors are in annualized percentage points.
    \end{figurenotes}
    \label{feSPF}
\end{figure}
...
\begin{align*}
\pi_t = & -\frac{\lambda_x}{\kappa}x_t + \frac{\lambda_x}{\kappa}\frac{(1-\alpha)\beta}{1-\alpha\beta} \bigg(k_t+f_{t|t-1}\mathbf{g}_{\pi,t} \bigg)\\
& \bigg(\E_t\sum_{i=1}^{\infty}x_{t+i}\prod_{j=0}^{i-1}(1-k_{t+1+j} - f_{t+j|t+j-1}\mathbf{g}_{\bar{\pi},t+j}) \bigg). 
...
Now simplify the target criterion further, assuming that learning has converged, $k_t = \mathbf{g}_{\pi} = 0$. We are left with 
\begin{equation}
...
\mathcal{L} &= \E_{t_0}\sum_{t=t_0}^{\infty} \beta^{t-t_0}\bigg\{ \pi_t^2  + \lambda x_t^2 + \varphi_{1,t} (\pi_t -\kappa x_t- \beta e_t +u_t) \\ &+ \varphi_{2,t}(e_t - e_{t-1} -k_t(\pi_t - e_{t-1})) + \varphi_{3,t}(k_t - \mathbf{g}(\pi_t - e_{t-1})) \bigg\},
 ...
 \begin{align}
  2\pi_t +2\frac{\lambda_x}{\kappa}x_t -\varphi_{2,t}(k_t + \mathbf{g}_{\pi}(\pi_t -e_{t-1}))& = 0 \label{simpleFOC1}, \\
...
\begin{figure}
    \subfigure[$i(\bar{\pi}, \text{all other states at their means})$]{\includegraphics[scale=\myTinyFigScale]{\myFigPath \fignameDiDpibar}} 
    \subfigure[Histogram of changes in $\bar{\pi}$]{\includegraphics[scale=\myTinyFigScale]{\myFigPath \fignameHistPib}} 
...
\begin{figure}[h!]
    \includegraphics[scale = \mySmallFigScale]{\myFigPath \fignameCBlossbaselineTwoY}
    
    \caption{Central bank loss as a function of $\psi_{\pi}$}
    \begin{figurenotes}
        Sample length is $T=100$ with a cross-section of $N=100$.
    \end{figurenotes}
    \label{fig_loss}
\end{figure}
...
\begin{figure}
    ...
    \begin{figurenotes}
        Cost-push shock imposed at $t=25$ of a sample length of $T=400$ (with 5 initial burn-in periods), cross-sectional average with a cross-section size of $N=100$.
    \end{figurenotes}
...
\begin{figure}
    ...
    \begin{figurenotes}
        Appendix \ref{unanchoring_in_data} discusses alternative measures of long-run inflation.
    \end{figurenotes}
    \label{rolling_pi}
\end{figure}
...
\begin{figure}
    \subfigure[Market-based inflation expectations from TIPS, various horizons]{\includegraphics[scale = \myTinyFigScale]{\myFigPath \fignameMarketEPiMoreHorizons}}
\subfigure[10-year ahead inflation expectations of firms (Livingston) and professional forecasters (SPF)]{\includegraphics[scale = \myTinyFigScale]{\myFigPath \fignameSPFLiv}}
    
    \caption{Alternative long-run inflation expectations measures}
    \begin{figurenotes}
        All inflation rates are annualized percentages. The series are at the following frequency: daily (10-year TIPS), monthly (20- and 30-year TIPS), quarterly (SPF), twice per year (Livingston).
    \end{figurenotes}
...
\begin{figure}
    \includegraphics[scale = \mySmallFigScale]{\myFigPath \fignameMarketEPiCleaned}
    
    \caption{Market-based inflation expectations, 10 year, average, \%}
    \begin{figurenotes}
        reakeven inflation, constructed as the difference between the yields of 10-year Treasuries and 10-year TIPS (blue line), difference between 10-year Treasury and 10-year TIPS, the latter cleaned from liquidity risk (red line).
    \end{figurenotes}
    \label{epi_cleaned}
\end{figure}
...
\end{pmatrix},\\
 A_s & = \begin{pmatrix} g_{\pi s} \\ g_{x s} \\ \psi_{\pi}g_{\pi s} + \psi_xg_{x s} + \begin{bmatrix} 0 & 1& 0\end{bmatrix}
\end{pmatrix}, \\
&\text{with} \\
...
\begin{align*}
\hat{\gamma} = &\bigg(\Omega^{data}-\frac{1}{N}\sum_{n=1}^N\Omega^{model}(\gamma, \Theta, \{e^n_t\}_{t=1}^{T})\bigg)' W^{-1} \numberthis
\label{GMM_loss}\\
&\bigg(\Omega^{data}-\frac{1}{N}\sum_{n=1}^N\Omega^{model}(\gamma, \Theta,  \{e^n_t\}_{t=1}^{T})\bigg), 
\end{align*}
...
\mathcal{L} &= \E_{t_0}\sum_{t=t_0}^{\infty} \beta^{t-t_0}\bigg\{  (\pi_t^2  + \lambda_x x_t^2 ) \numberthis \\
 & + \varphi_{1,t} \bigg(\pi_t - \kappa x_t -(1-\alpha)\beta f_{a,t} -\kappa\alpha\beta b_2 (I_3 - \alpha\beta h)^{-1}s_t - e_3(I_3 - \alpha\beta h)^{-1}s_t \bigg) \\
 & + \varphi_{2,t} \bigg(x_t + \sigma i_t -\sigma f_{b,t}  -  (1-\beta)b_2 (I_3 - \beta h)^{-1}s_t + \sigma\beta b_3 (I_3 - \beta h)^{-1}s_t \\
 & -\sigma e_1(I_3 - \beta h)^{-1}s_t  \big)\bigg) \\
...
& \varphi_{6,t} + \E_t\varphi_{6,t+1} = f_{t|t-1} \varphi_{5,t} \label{constraints_general},
\end{align}
where the multiplier $\varphi_{6,t+1}$ is the new element vis-\`a-vis the case where the anchoring function is specified in levels ($k_t = \mathbf{g}(f_{t|t-1})$), and I am using the shorthand notation
\begin{align}
...
\begin{align*}
\varphi_{6,t} & = -c f_{t|t-1}\E_t x_{t+1} + \E_t\bigg(1+ \frac{f_{t|t-1}}{f_{t+1|t}}(1-k_{t+1}) -f_{t|t-1} \mathbf{g}_{\bar{\pi},t} \bigg) \varphi_{6,t+1} \numberthis \label{6'} \\
 & -\E_t\frac{f_{t|t-1}}{f_{t+1|t}}(1-k_{t+1})\varphi_{6,t+2},  \\
0 & = 2\pi_t + 2\frac{\lambda_x}{\kappa}x_t   - \bigg( \frac{k_t}{f_{t|t-1}} + \mathbf{g}_{\pi,t}\bigg)\varphi_{6,t} + \frac{k_t}{f_{t|t-1}}\E_t\varphi_{6,t+1} \numberthis \label{1'}.
\end{align*}
...
f_{b,t} & = \frac{1}{1-\beta}\bar{\pi}_{t-1}  + b(I_{3} - \beta h)^{-1}s_t  \label{A8} \numberthis ,\\
 ...
 \pi_t & = -\frac{\lambda_x}{\kappa}\bigg\{x_t - \frac{(1-\alpha)\beta}{1-\alpha\beta} \bigg(k_t+f_{t|t-1}\mathbf{g}_{\pi,t} \bigg) \numberthis \label{B1} \\
 & \bigg(\E_t\sum_{i=1}^{\infty}x_{t+i}\prod_{j=0}^{i-1}(1-k_{t+1+j} - f_{t+1+j|t+j}\mathbf{g}_{\bar{\pi}, t+j}) \bigg)
\bigg\} .
\end{align*}
...
\begin{figure}
    \includegraphics[scale=\mySmallFigScale]{\myFigPath \fignamePEAvsVFIfirstX}
