...
\label{anchoring}
\end{equation}
where $\mathbf{g(\cdot)}$ is a smooth function. I refer to $ \mathbf{g}(\cdot)$ as the anchoring function. Notice that this function is a extension of the conventional decreasing or constant gain specifications, as it nests both as special cases. A decreasing gain implies $\mathbf{g}(k_{t-1},f_{t|t-1})  = (k_{t-1}^{-1}+1)^{-1}$ while a constant gain sets $\mathbf{g}(k_{t-1},f_{t|t-1})  = \bar{g}$. Furthermore, in both special cases, $g_2 = 0$.

I make the following assumptions on the function $\mathbf{g(\cdot)}$:
...
\end{align}
 ...
 Appendix \ref{alternative_criteria} compares alternative specifications for $\mathbf{g(\cdot)}$ and discusses the motivation behind using specific functional forms. I let my choice of functional form for $\mathbf{g(\cdot)}$ be guided by data, wherefore I start my inquiry by estimating the anchoring function (see Section \ref{estimation}).
...
\label{ALM}
\end{equation}
where $g^l$ is a $3\times4$ matrix given in Appendix \ref{app_FG}. Thus, instead of the state-space solution of the RE version of the model (Equations (\ref{LOM_s}) and (\ref{RE_obs})), the state-space solution for the learning model is characterized by the pair of equations (\ref{LOM_s}) and (\ref{ALM}). 
