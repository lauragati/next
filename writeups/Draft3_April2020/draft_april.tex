...
Like in time-zero optimal commitment under rational expectations, optimal policy will be time-inconsistent. But not only that. I also find that optimal policy in the anchoring model, as in any adaptive learning setting, is not history-dependent: the Ramsey problem does not have a commitment solution. As observed also by \cite{mele2019perils}, who come to a similar conclusion in a decreasing gain learning model, this is due to the fact that backward-looking expectation formation cannot function as a commitment device for the policymaker. 
...
\end{equation}
The Lagrange multipliers $\varphi_5 \geq 0$ and $\varphi_6 \geq 0$ are the multipliers of the RLS updating and the anchoring equation respectively. This equation, upon substitution of the solutions for the two multipliers, yields the target criterion and is thus ideal for interpretation. First, since $\varphi_{5,t}k_t^{-1} > 0$, one immediately observes the above-discussed conclusion that as long as the adaptive learning equation is a constraint to the policymaker ($\varphi_{5,t} > 0$), the central bank has more room to transfer the contemporaneous tradeoff between inflation and the output gap to the future.
