...
\begin{align*}
k_t^{-1} & = \begin{cases} (k_{t-1}+1)^{-1} \quad \text{if} \quad \theta_t < \tilde{\theta}  \\ \bar{g}  \quad \text{otherwise.}\numberthis \label{anchoring_kinked}
\end{cases} 
\end{align*}
...
\begin{align}
& 2\pi_t + 2\frac{\lambda_x}{\kappa}x_t -\varphi_{5,t} k_t^{-1} - \varphi_{6,t} \mathbf{g}_{\pi,t} = 0 \label{gaspar22}\\
& -\frac{2(1-\alpha)\beta}{1-\alpha\beta}\frac{\lambda_x}{\kappa}x_{t+1} + \varphi_{5,t} -(1-k_t^{-1})\varphi_{5,t+1} +\mathbf{g}_{\bar{\pi},t}\varphi_{6,t+1} = 0 \label{gaspar21}\\
...
\begin{align}
& 2\pi_t + 2\frac{\lambda_x}{\kappa}x_t -k_t^{-1} \varphi_{5,t} - \mathbf{g}_{\pi,t}\varphi_{6,t}  = 0 \label{gaspar22_general}\\
...
\end{align}
where the blue multiplier is the new element vis-\`a-vis the case where the anchoring function is specified in levels ($k_t^{-1} = \mathbf{g}(fe_{t|t-1})$), and I'm using the shorthand notation
\begin{align}
...
0 & = 2\pi_t + 2\frac{\lambda_x}{\kappa}x_t   - \bigg( \frac{k_t^{-1}}{fe_{t|t-1}} + \mathbf{g}_{\pi,t}\bigg)\varphi_{6,t} + \frac{k_t^{-1}}{fe_{t|t-1}}\varphi_{6,t+1}\label{1'}
\end{align}
