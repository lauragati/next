...
\title{Materials 11 - trying to get a reasonable gain $\bar{g}$}
\author{Laura G\'ati} 
\date{\today}
\maketitle
...
\begin{align}
FE_{t-1} & = z_{t}-\hat{\E}_{t-1}(z_{t})\quad \text{one-period-ahead forecast error realized at time } t \\
& = ALM(t) - PLM(t-1) \\
...
\item \textbf{Evening}: Update $\bar{z}_t = \bar{z}_{t-1} + k_t^{-1}(z_{t}-(\bar{z}_{\color{red}{t-1}} + bs_{t-1}))$
\item[] $\rightarrow$ evening of time $t$ available: $\mathcal{I}_t^e = \{\bar{z}_{\color{red}{t}}, s_t, k_{t}, FE_{t-1}\}$
\end{enumerate}

\newpage
\section{Trying to get an optimal gain}

$T = 2000, N=100$. Note: MSE as a function of gain is convex. Stats:

\begin{multicols}{2}
DGP is learning with constant gain $\bar{g} = 0.1450$. 
\begin{itemize}
\item $Mean(\bar{g}^*_n) = -0.00028929$
\item $Var(\bar{g}^*_n) = 1.4561e-06$
\item \% of $\bar{g}^*_n < 0$ is 68\%.
\end{itemize}

\columnbreak
DGP is RE.
\begin{itemize}
\item $Mean(\bar{g}^*_n) = -0.00021913$
\item $Var(\bar{g}^*_n) = 1.0134e-06$
\item \% of $\bar{g}^*_n < 0$ is 67\%.
\end{itemize}
...
\section{Susanto's points and big picture}

\begin{itemize}
\item L. Ball: credible disinflation causes booms in NK model (1994)
\item instrument instability (Holbrook 1972, Sims 1974, Lane 1984)
\end{itemize}
