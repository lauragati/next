...
Main sources: Chari et al 2000, Woodford 2003, Nakamura \& Steinsson 2008, Carvalho et al 2019
...
\caption{Time series of forecast errors, changes in long-run expectations and implied gain}
\label{fe_in_data}
\end{figure}
...
Let $\; \mathbf{g}_{z,t} \equiv \frac{\partial \mathbf{g}}{\partial z}\;$ at $t$ and define $b_1$ as the first row of $b$. Then monetary policy optimally brings about the following target relationship between inflation and the output gap
...
\onslide<3>{\textcolor<3>{brownreddark}{Endeogenous gain: ability to move $\E_t x_{t+i}$ depends on present and future degree of unanchoring}}
...
\only<1-3>{5 bp movement in $\bar{\pi}$ $\rightarrow $ 250 bp movement in $i$} \only<2-3>{(1 standard dev. change in $\bar{\pi}$)}

\

\only<3>{Mode: 0.3 bp movement in $\bar{\pi}$ $\rightarrow $ 15 bp movement in $i$}
...
\begin{figure}[h!]
\includegraphics[scale = 0.22]{\myFigPath \fignameIRFanchUnanchTogether}
...
\pause
\small{
\textcolor{dodgerblue}{Anchoring-optimal coefficient: $\psi_{\pi}^A =1.09$} $\quad \quad $\textcolor{brownreddark}{ RE-optimal coefficient: $\psi_{\pi}^{RE} =2.21$}
}
...
\includegraphics[scale = 0.22]{\myFigPath \fignameIRFpsipiTogether}
...
\item[] IS- and Phillips curve:	
 \begin{align*}
x_t &=  -\sigma \textcolor<1-3>{dodgerblue}{i_t} +\hat{\E}_t \sum_{T=t}^{\infty} \beta^{T-t }\big( (1-\beta)x_{T+1} - \sigma(\beta \textcolor<2-3>{brownreddark}{i_{T+1}} - \pi_{T+1}) +\sigma r_T^n \big)   \\
...
\begin{itemize}
\item<1-3>  \textcolor{dodgerblue}{Current interest rate $i_t$: one channel of policy}

\

\item<2-3>  \textcolor{brownreddark}{Expected future interest rates: additional channel of policy}

\

 
\item<3>  \textcolor{brownreddark}{Only if policy reaction function internalized}
\end{itemize}
