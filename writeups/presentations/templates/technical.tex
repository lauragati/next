...
\begin{equation}
\hat{\E}_t y_{t+1} = \begin{pmatrix} \textcolor{brownreddark}{\bar{\pi}_{t-1}} \\ 0 \\ 0 \end{pmatrix} + g h s_t  \label{PLM_fcst_general}
\end{equation}
...
\item[] \textcolor{brownreddark}{$\bar{\pi}_{t-1}$}	$\rightarrow$ concept of long-run inflation expectations in the model
...
\item Estimate $\bar{\pi}_{t-1}$ using recursive least squares (RLS) using observed states and knowledge of (\ref{state})
...
\
\pause
$\rightarrow$ Recursive least squares update for long-run inflation expectations:	
...
%%%%%%% Slide %%%%%%
\begin{frame}[plain]  % remove progress bar from appendix
	\frametitle{Further evidence: disagreement}\label{further_evidence}
...
%%%%%%% Slide %%%%%%
\begin{frame}[plain]  % remove progress bar from appendix
%	\frametitle{PCE core against target}\label{further_evidence}
