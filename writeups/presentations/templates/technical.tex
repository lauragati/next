...
\begin{figure}[h!]
\caption{Expectations of average inflation over 10 years}
\includegraphics[scale = 0.25]{\myFigPath \fignameSPFEPi}
\label{epi}
\end{figure}
...
%	\item[$\hookrightarrow$]  E.g. for a 0.1 pp fall in long-run inflation expectations, lowers interest rate by \movei pp
	...
	\item[$\hookrightarrow$] Anchoring-optimal Taylor rule eliminates 90\% of loss from volatility
...
\includegraphics[scale = 0.24]{\myFigPath \fignameEpiData}
\label{pi_epi}
\end{figure}
...
Second half of sample: estimate

\begin{align*}
\bar{\pi}_t = & \beta_0 + \beta_1 fe_{t|t-1} + \epsilon_t \numberthis \\
\\
\bar{\pi}_t & \equiv \E_t(\pi_{t+10})\numberthis \\
fe_{t|t-1} & \equiv \pi_t - \E_{t-1}(\pi_t) \numberthis
\end{align*}
...
$\hat{\beta}_1 = 0.04 \quad $  (p-value: 0.0016)

\


1 pp forecast error $\rightarrow$ 4 bp revision in long-run expectations
\end{frame}
%%%%%%%%%%%%%%%%%
...
Picked to match the autocovariances of inflation, output gap, interest rate and one-period ahead inflation expectations for lags $0, \dots, 4$.
\end{frame}
%%%%%%%%%%%%%%%%%%
...
%\hfill
%\subfigure[Taylor rule with 1.5 and 0.3 coefficients on inflation and the output gap]{\includegraphics[scale=0.148]{\myFigPath \fignamePEAobsTR}}
...
\includegraphics[scale=0.28]{\myFigPath \fignameCBlossbaseline}
\end{figure} 
...
Anchoring-optimal coefficient: $\psi_{\pi}^A =1.1$
...
$\rightarrow$ If model is anchoring, anchoring-optimal $\psi_{\pi}^A$ gets 90\% of the distance to RE-optimal $\psi_{\pi}^{RE}$ under RE
