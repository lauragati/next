...
%
%
%11/27/2012: Modified by Ryan Chahrour to work with model structures, as declared in model.m
...
%This is the derivative of the SS values of variables

if isfield(mod, 'adiff') && mod.adiff
    
    xss = mod.Xss;
    yss = mod.Yss;

    %This the gam from Ryan's notes
    GAM = subs([fxp fyp fx fy], [x xp y yp], log([xss xss yss yss])); %[A1 B1 A2 B2]
    mod.dgam_dtheta = jacobian(GAM(:),transpose(mod.PARAM)); %Jacobian with respect to parameters
    
    %This is change in eta withspect to param
    mod.deta = jacobian(mod.shck(:),mod.PARAM);
    
    %This is change in R with respect to param
    mod.dR = jacobian(mod.me(:), mod.PARAM);
end
