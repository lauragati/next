% a version of sim_learnLH.m that's for use for the exercise of
% approximating the reaction function ONLY. I'm not changing much, just
% simplifying outputs
% 9 April 2020
function [xsim, ysim, phi_seq, k, g_pi, g_pibar] = sim_learnLH_approx_reaction(gx,hx,eta,T,ndrop,e, Aa, Ab, As, param, PLM, gain)

this_code = mfilename;
max_no_inputs = nargin(this_code);

% I won't allow shocks in this exercise - we're not computing IRFs
dt = 0;
x0 = 0;
...
%                 [fk, om, thet] = fk_cusum(param,k(:,t-1),om, thet,fe);
            elseif crit == 3 % smooth criterion
                ...
                [fk, g_pi(t), g_pibar(t)] = fk_smooth_pi_only(param,fe,k(:,t-1));
            end
            k(:,t) = fk;
