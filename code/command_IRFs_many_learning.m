...
for s=2  %2->zoom in on monetary policy shock
    ...
    for n=1:N
        ...
        % Unshocked
        dbstop in sim_learn_EE at 47 if isnan(det(R))==1
        % LH learning (learning both slope and constant of a vector)
        [~, y_LH] = sim_learnLH(gx,hx,SIG,T,burnin,e, Aa, Ab, As, param, PLM, cgain);
        ...
        for t=1:nd
            dt = dt_vals(t);
            % Shocked
            [~, ys_LH] = sim_learnLH(gx,hx,SIG,T,burnin,e, Aa, Ab, As, param, PLM, cgain, dt, x0);
            [~, ys_EE] = sim_learn_EE(gx,hx,fxpn,fxn,fypn,fyn,SIG,T,burnin,e, dt, x0);
...
for t=1:nd % for the two diff times of imposing the shock
    ...
    series(1,:,:) = RIR_Y_EE(:,:,t)';
    series(2,:,:) = iry';
    figname = [this_code, '_', 'RIR_EE_' shocknames{s}, PLM_name,'_rho',rho_val, '_psi_pi_', psi_pi_val, '_sig_', sig_val, '_dt_', num2str(dt), '_gbar_', gbar_val];
    ...
    series(1,:,:) = RIR_Y_LH(:,:,t)';
    series(2,:,:) = iry';
    figname = [this_code, '_', 'RIR_LH_' shocknames{s}, PLM_name,'_rho',rho_val, '_psi_pi_', psi_pi_val, '_sig_', sig_val, '_dt_', num2str(dt), '_gbar_', gbar_val];
    subplot_names = titles_obs;
    legendnames = {'Learning', 'RE'};
