...
anal = 1; % take analytical LR exp
[x_LR_pidrift, y_LR_pidrift, ~, diff_pidrift, pibar, kd] = sim_learn_LR_anchoring_pidrift(gx,hx,SIG,T,burnin,e, Aa_LR, Ab_LR, As_LR, param, setp, H, anal);
kd_inv = 1./kd;
V(:,:,5) =y_LR_pidrift;
%% Plots
% Observables
...
close all
% the learning inflation drift only graphs
...
for j=1:ny
    ...
    plot(squeeze(V(j,end-100:end,3)),'linewidth', 2)
    plot(squeeze(V(j,end-100:end,4)),'linewidth', 2)
    plot(squeeze(V(j,end-100:end,5)),'linewidth', 2)
    ax = gca; % current axes
    ax.FontSize = fs;
    grid on
    grid minor
    legend('RE', 'EE','LR', 'LR anchoring', 'LR anchoring with only \pi drift','location', 'southoutside')
    title(titles(l))
end
if print_figs ==1
    figname = ['materials3_observables_anchoring_pidrift_last100']
    cd(figpath)
    export_fig(figname)
    cd(current_dir)
    close
...
title('Inverse gain')
if print_figs ==1
    figname = ['materials3_observables_anchoring_pidrift_gain']
    cd(figpath)
    export_fig(figname)
    cd(current_dir)
    close
...
% full sample, compare vector and scalar anchoring
figure
set(gcf,'color','w'); % sets white background color
set(gcf, 'Position', get(0, 'Screensize')); % sets the figure fullscreen
...
for j=1:ny
    ...
    plot(squeeze(V(j,:,1)),'linewidth', 2); hold on
    plot(squeeze(V(j,:,4)),'linewidth', 2)
    plot(squeeze(V(j,:,5)),'linewidth', 2, 'linestyle','--')
    ax = gca; % current axes
    ax.FontSize = fs;
    grid on
    grid minor
    legend('RE','LR anchoring', 'LR anchoring with only \pi drift','location', 'southoutside')
    title(titles(l))
end
if print_figs ==1
    figname = ['materials3_observables_anchoring_pidrift_last100_compare_anchoring']
    cd(figpath)
    export_fig(figname)
    cd(current_dir)
    close
