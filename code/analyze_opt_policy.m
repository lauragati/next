...
% Compute mean and median pibar 
mean_pib = mean(pibsim)
sort_pib = sort(pibsim);
if mod(T,2)==1
    median_pib = sort_pib(ceil(T/2));
else
    median_pib = (sort_pib(T/2)+sort_pib(T/2+1))/2
end
std(pibsim)
...
% changes in pibsim
dpib = pibsim(2:end)-pibsim(1:end-1);
plot(dpib)
mean_dpib = mean(dpib)
sort_dpib = sort(dpib);
if mod(T,2)==1
    median_dpib = sort_dpib(ceil(T/2));
else
    median_dpib = (sort_dpib(T/2)+sort_dpib(T/2+1))/2
end
std_dpib = std(dpib)


hist_dpib = histogram(dpib, 48) 
hist_dpib.BinEdges % the most frequent is the  [-0.0037    0.0034] bin (the mode of the bins)
max(hist_dpib.Values)/length(dpib)  % about 25% of values are in this bin
close

% PLOT HISTOGRAM OF CHANGES IN PIBAR %%%%%%%%%%%%%%
% Plot configs
[fs, lw] = plot_configs;
...
grid on
grid minor
xl = xlabel('$\bar{\pi}$, percentage points','interpreter', 'latex', 'fontsize', fs*4/5);
% xl.Position(1) = xlplus(1) + xl.Position(1);
% xl.Position(2) = xlplus(2) + xl.Position(2);
% 
% 
yl = ylabel('$i(\bar{\pi}, \cdot)$, percentage points','interpreter', 'latex', 'fontsize', fs*4/5);
% yl.Rotation = 0; % rotate
% yl.Position(1) = ylplus(1) + yl.Position(1); % move left
% yl.Position(2) = ylplus(2) + yl.Position(2); % move up

figname = [this_code, '_ip', todays_date];
if print_figs ==1
    disp(figname)
    cd(figpath)
    export_fig(figname)
