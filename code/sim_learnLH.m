% simulate data from long-horizon learning model, vector learning
% PLM = 1 if learn constant only, -1 if "only-mean" learning, 2 if both constant and slope.
% PLM = 11, -11 or 21 if only pi is learned, the first digit pertains to the above.
% gain: 1 = decreasing gain, 3 = constant gain, 21 = endog gain, CEMP 22 =
% endog gain, CUSUM, 23 = endog gain, smooth criterion (only when only pi is learned)
% learn_selector = [1,1,1]' if pi,x,i learned, = [1,0,0]' if only pi is learned
...
if PLM==1 || PLM == 2 || PLM==11 || PLM == 21
    ...
    % 11 = learning only a constant, just for inflation
