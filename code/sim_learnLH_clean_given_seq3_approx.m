...
% by FE
% this version uses the approximated univariate LOMgain from the GMM estimation, hence
% "approx"
% 13 June 2020
function [xsim, ysim, k, phi_seq, FA, FB, fe, g_pi, g_pibar,diff] = sim_learnLH_clean_given_seq3_approx(param,gx,hx,eta,PLM,gain,T,ndrop,e,seq,n_input_jumps,...
    alph,x,fegrid, g_fe, knowTR, dt, x0)
...
%                 [fk, g_pi(t), g_pibar(t)] = fk_smooth_pi_only(param,fe(t),k(:,t-1));
                [fk, g_pi(t), g_pibar(t)] =fk_smooth_approx_univariate(alph,x,fe(t),fegrid, g_fe); 
            end
            k(:,t) = fk;
            if fk < 0
                warning('k<0')
