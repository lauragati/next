% evaluate the target criterion in the midsimple anchoring model
% 3 April 2020
% updated 4 April 2020
function resid = eval_resTC(param,hx,ysim,k,phi_seq,s,g_pi,H)
lamx = param.lamx;
kapp = param.kapp;
bet  = param.bet;
alph = param.alph;
...
for i=2:H-1
    prod = 1;
    for j=1:i-1
%         prod = prod*(1-k(t+1+j)^(-1)-(pi(t+1+j)-pibar(t+j)-b(1,:)*s(:,t+j))*g_pi(t+j)); % this assumed perfect foresight
